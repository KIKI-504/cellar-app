...
function printLabel(s) {
  ...
  const dp = s.dp_price ? parseFloat(s.dp_price).toFixed(2) : s.wines?.purchase_price_per_bottle ? ((parseFloat(s.wines.purchase_price_per_bottle) + dutyForSize(s.bottle_size)) * 1.2).toFixed(2) : null
...
export default function StudioPage() {
  ...
  function calcDP(ibPrice, bottleSize) {
    ...
    return ((parseFloat(ibPrice) + dutyForSize(bottleSize || '75')) * 1.2).toFixed(2)
  }
  ...
  async function updateWine(studioId, wineId, field, value) {
    ...
    if (!error) { setStudioWines(prev => prev.map(s => s.id === studioId ? { ...s, wines: { ...s.wines, [field]: value } } : s)); flashSave() }
  }
  ...
  async function updateWsPrice(wineId, studioId, wsValue) {
    ...
    const { error } = await supabase.from('wines').update({ ws_lowest_per_bottle: wsValue, ws_price_date: today }).eq('id', wineId)
  ...
  async function searchWines(q) {
    ...
    const { data } = await supabase.from('wines').select('id, description, vintage, colour, region, purchase_price_per_bottle, quantity').ilike('description', `%${q}%`).order('description').limit(10)
    setMoveResults(data || [])
  }
  ...
  async function confirmMove() {
    ...
    const { error } = await supabase.from('studio').insert({ wine_id: selectedWine.id, quantity: moveQty, date_moved: moveDate, dp_price: dp, status: 'Available', notes: moveNotes || null, include_in_local: false })
  ...
  function updateAddField(field, value) {
    ...
    if (['addDescription','addProducer','addVintage','addColour','addBottleSize'].includes(field)) {
      setAddWineId(generateWineId(field==='addVintage'?value:fields.addVintage,field==='addProducer'?value:fields.addProducer,field==='addDescription'?value:fields.addDescription,field==='addColour'?value:fields.addColour,field==='addBottleSize'?value:fields.addBottleSize))
    }
    if (field==='addDescription') setAddDescription(value); if (field==='addProducer') setAddProducer(value)
    if (field==='addVintage') setAddVintage(value); if (field==='addColour') setAddColour(value)
    if (field==='addRegion') setAddRegion(value); if (field==='addCountry') setAddCountry(value)
    if (field==='addBottleSize') setAddBottleSize(value); if (field==='addQuantity') setAddQuantity(value)
    if (field==='addIBPrice') setAddIBPrice(value); if (field==='addSalePrice') setAddSalePrice(value)
    if (field==='addWsPrice') setAddWsPrice(value); if (field==='addNotes') setAddNotes(value)
  }
  ...
  async function saveNewWine() {
    ...
    try {
      ...
      const dp = ibPrice ? ((ibPrice + dutyForSize(addBottleSize)) * 1.2).toFixed(2) : null
      const wsPrice = addWsPrice ? parseFloat(addWsPrice) : null
      const today = new Date().toISOString().split('T')[0]
      const { data: newWine, error: wineError } = await supabase.from('wines').insert({
        source:'Manual', source_id:addWineId||null, description:addDescription, vintage:addVintage||null,
        colour:addColour||null, region:addRegion||null, country:addCountry||null,
        bottle_format:addBottleSize==='150'?'Magnum':addBottleSize==='37.5'?'Half Bottle':'Bottle',
        bottle_volume:addBottleSize, quantity:String(addQuantity), purchase_price_per_bottle:ibPrice,
        include_in_buyer_view:false, ws_lowest_per_bottle:wsPrice, ws_price_date:wsPrice?today:null,
      }).select().single()
      if (wineError) throw wineError
      const { error: studioError } = await supabase.from('studio').insert({
        wine_id:newWine.id, quantity:addQuantity, date_moved:today, dp_price:dp,
        sale_price:addSalePrice?parseFloat(addSalePrice):null, status:'Available',
        notes:addNotes||null, include_in_local:false, bottle_size:addBottleSize, colour:addColour||null,
      })
  ...
  function openScanModal() {
    setShowScanModal(true); setScanFile(null); setScanPreview(null); setScanAnalysing(false); setScanDone(false)
    ...
    setScanVintage(''); setScanBottleSize('75'); setScanSalePrice(''); setScanColour(''); setScanIBPrice(''); setScanRetailPrice('')
  }
  ...
  function handleScanFileSelect(e) {
    ...
    const reader = new FileReader(); reader.onload = ev => setScanPreview(ev.target.result); reader.readAsDataURL(file)
  }
  ...
  async function analyseLabel() {
    ...
    try {
      const base64 = await new Promise((res, rej) => { const r = new FileReader(); r.onload = () => res(r.result.split(',')[1]); r.onerror = rej; r.readAsDataURL(scanFile) })
      const response = await fetch('/api/analyse-label', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ imageBase64:base64, mediaType:scanFile.type }) })
      ...
      if (searchTerm) {
        ...
        if (extracted.vintage) {
          const { data } = await supabase.from('wines').select('id, description, vintage, colour, region, purchase_price_per_bottle, quantity').ilike('description', `%${searchTerm}%`).eq('vintage', extracted.vintage).limit(5)
          matches = data
        }
        if (!matches || matches.length === 0) {
          const { data } = await supabase.from('wines').select('id, description, vintage, colour, region, purchase_price_per_bottle, quantity').ilike('description', `%${searchTerm}%`).limit(5)
          matches = data
        }
        if (matches && matches.length > 0) {
    ...
    } catch (err) { alert('Label reading failed: ' + err.message) }
    setScanAnalysing(false)
  }

  async function scanSearchWines(q) {
    setScanSearch(q); if (q.length < 2) { setScanSearchResults([]); return }
    const { data } = await supabase.from('wines').select('id, description, vintage, colour, region, purchase_price_per_bottle, quantity').ilike('description', `%${q}%`).order('description').limit(8)
  ...
  const filtered = studioWines
    ...
    .sort((a, b) => {
      ...
      else if (sortField === 'vintage') { av = a.wines?.vintage || a.unlinked_vintage || ''; bv = b.wines?.vintage || b.unlinked_vintage || '' }
      else if (sortField === 'region') { av = (a.wines?.region || '').toLowerCase(); bv = (b.wines?.region || '').toLowerCase() }
      else if (sortField === 'bottle_size') { av = bottleSortKey(a.bottle_size); bv = bottleSortKey(b.bottle_size) }
    ...
    <div style={{ minHeight: '100vh', background: 'var(--cream)', overflowX: 'hidden' }}>
      ...
      <div style={{ padding: '76px 28px 24px' }}>
        ...
        <div style={{ display:'flex', gap:'10px', marginBottom:'8px', flexWrap:'wrap', alignItems:'center' }}>
          ...
          <select value={filterColour} onChange={e => setFilterColour(e.target.value)} style={{ border:'1px solid var(--border)', background:'var(--white)', padding:'8px 10px', fontFamily:'DM Mono, monospace', fontSize:'11px', outline:'none' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
            ...
            <tbody>
              {filtered.map(s => {
                ...
                const dpVal = s.dp_price ? parseFloat(s.dp_price).toFixed(2) : s.wines?.purchase_price_per_bottle ? ((parseFloat(s.wines.purchase_price_per_bottle) + dutyForSize(s.bottle_size)) * 1.2).toFixed(2) : null
                const ws = s.wines?.ws_lowest_per_bottle ? parseFloat(s.wines.ws_lowest_per_bottle) : null
                const wsDate = s.wines?.ws_price_date || null
                const wsDP = ws ? ((ws + dutyForSize(s.bottle_size)) * 1.2).toFixed(2) : null
      ...
      {showScanModal && (
        <div style={{ position:'fixed', inset:0, background:'rgba(20,15,10,0.7)', zIndex:500, display:'flex', alignItems:'center', justifyContent:'center', padding:'20px' }}>
          <div style={{ background:'var(--white)', width:'100%', maxWidth:'560px', maxHeight:'90vh', overflowY:'auto', padding:'28px' }}>
            ...
            {scanDone && (<div style={{ background:'var(--cream)', padding:'12px', marginBottom:'16px', fontSize:'11px', fontFamily:'DM Mono, monospace', color:'var(--muted)' }}>Detected: {[scanRaw?.vintage, scanRaw?.wine_name, scanRaw?.producer].filter(Boolean).join(' · ')}</div>)}
            {(scanDone || scanFile) && (
              <div>
                {scanWine ? (
                  <div style={{ background:'var(--cream)', padding:'10px 12px', marginBottom:'12px', fontSize:'12px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                    <div><div style={{ fontFamily:'Cormorant Garamond, serif', fontSize:'15px' }}>{scanWine.description}</div><div style={{ fontSize:'10px', color:'var(--muted)', marginTop:'2px' }}>{scanWine.vintage} · {scanWine.region}</div></div>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px', marginBottom:'12px' }}>
                  <div><label style={labelStyle}>Qty</label><input type="number" min="1" value={scanQty} onChange={e => setScanQty(parseInt(e.target.value)||1)} style={inputStyle} /></div>
                  <div><label style={labelStyle}>Date Added</label><input type="date" value={scanDate} onChange={e => setScanDate(e.target.value)} style={inputStyle} /></div>
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(20,15,10,0.7)', zIndex:500, display:'flex', alignItems:'center', justifyContent:'center', padding:'20px' }}>
          <div style={{ background:'var(--white)', width:'100%', maxWidth:'600px', maxHeight:'90vh', overflowY:'auto', padding:'28px' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'14px', marginBottom:'14px' }}>
              ...
              <div><label style={labelStyle}>Quantity</label><input type="number" min="1" value={addQuantity} onChange={e => updateAddField('addQuantity',parseInt(e.target.value)||1)} style={inputStyle} /></div>
              <div><label style={labelStyle}>IB Price / btl £</label><input type="number" step="0.01" value={addIBPrice} onChange={e => updateAddField('addIBPrice',e.target.value)} placeholder="0.00" style={inputStyle} /></div>
              <div><label style={labelStyle}>Sale Price £</label><input type="number" step="0.01" value={addSalePrice} onChange={e => updateAddField('addSalePrice',e.target.value)} placeholder="0.00" style={inputStyle} /></div>
              <div><label style={labelStyle}>WS Ex-Duty Price / btl £</label><input type="number" step="0.01" value={addWsPrice} onChange={e => updateAddField('addWsPrice',e.target.value)} placeholder="0.00" style={inputStyle} /></div>
              <div style={{ gridColumn:'1 / -1' }}><label style={labelStyle}>Notes</label><input value={addNotes} onChange={e => updateAddField('addNotes',e.target.value)} placeholder="Optional delivery note" style={inputStyle} /></div>
              {addWineId && (<div style={{ gridColumn:'1 / -1' }}><label style={labelStyle}>Wine ID (auto-generated)</label><div style={{ fontFamily:'DM Mono, monospace', fontSize:'11px', color:'var(--muted)', padding:'6px 0' }}>{addWineId}</div></div>)}
            </div>
