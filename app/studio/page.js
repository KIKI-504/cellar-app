...
// ─── Status badge for Sold / Consumed rows ────────────────────────────────────
function StatusBadge({ status }) {
  if (status !== 'Sold' && status !== 'Consumed') return null
  ...
  return (
    <span style={{
      ...
      color: isSold ? '#c0392b' : '#7a5e10',
      border: `1px solid ${isSold ? 'rgba(192,57,43,0.35)' : 'rgba(122,94,16,0.35)'}`,
      padding: '1px 5px',
      borderRadius: '2px',
...
export default function StudioPage() {
  ...
  function calcDP(ibPrice, bottleSize) {
    ...
    const duty = (String(bottleSize) === '150') ? 6 : 3
  ...
  async function confirmMove() {
    ...
    setMoveSaving(true)
    const dp = calcDP(selectedWine.purchase_price_per_bottle, '75')
  ...
  async function saveNewWine() {
    ...
    try {
      ...
      const dp = ibPrice ? calcDP(ibPrice, addBottleSize) : null
  ...
  async function saveScanEntry() {
    ...
    const dp = ibPrice ? calcDP(ibPrice, scanBottleSize) : null
  ...
  // ─── Filter + Sort ────────────────────────────────────────────────────────────
  const BOTTLE_SIZE_ORDER = { '37.5': 0, '75': 1, '150': 2, '300': 3 }
  ...
  const filtered = studioWines
    ...
    .sort((a, b) => {
      ...
      if (sortField === 'quantity') {
        ...
        av = BOTTLE_SIZE_ORDER[String(a.bottle_size || '75')] ?? 1
        bv = BOTTLE_SIZE_ORDER[String(b.bottle_size || '75')] ?? 1
      } else {
      ...
      <div style={{ padding: '76px 28px 24px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '16px', flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <select value={`${sortField}:${sortDir}`}
            ...
            <option value="bottle_size:asc">Size: small → large</option>
            <option value="bottle_size:desc">Size: large → small</option>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
            ...
            <tbody>
              ...
              {filtered.map(s => {
                ...
                const isSoldOrConsumed = s.status === 'Sold' || s.status === 'Consumed'
                ...
                return (
                  ...
                  <tr style={{
                    borderBottom: isDetailOpen ? 'none' : '1px solid #ede6d6',
                    background: s.include_in_local ? 'rgba(45,106,79,0.04)' : 'var(--white)',
                    opacity: isSoldOrConsumed ? 0.55 : 1,
                  }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '6px' }}>
                        ...
                        <div>
                          {isEditing && !w ? (
                            ...
                            <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '14px', lineHeight: 1.3, fontWeight: 500 }}>
                              {winePart}
                    ...
                    <td style={{ padding: '9px 12px' }}>
                      {isEditing ? (
                        <EditableCell type="number" step="0.01"
                          value={s.dp_price ? parseFloat(s.dp_price) : (w?.purchase_price_per_bottle ? parseFloat(calcDP(w.purchase_price_per_bottle, s.bottle_size || '75')) : null)}
                        ...
                        <span style={{ fontFamily: 'DM Mono, monospace', fontSize: '12px', fontWeight: 600 }}>
                          {s.dp_price ? `£${parseFloat(s.dp_price).toFixed(2)}` : w?.purchase_price_per_bottle ? `£${calcDP(w.purchase_price_per_bottle, s.bottle_size || '75')}` : '—'}
                        </span>
                      )}
                    </td>
                      ...
                      <td colSpan={13} style={{ padding: '8px 20px 12px 36px' }}>
                        <div style={{ display: 'flex', gap: '28px', fontSize: '11px', fontFamily: 'DM Mono, monospace', flexWrap: 'wrap' }}>
                          ...
                          {s.dp_price && s.sale_price && (
                            <div><span style={{ color: 'var(--muted)', letterSpacing: '0.08em', textTransform: 'uppercase', fontSize: '9px' }}>Margin on DP</span>
          ...
          <div style={{ background: 'var(--cream)', width: '100%', maxWidth: '520px', padding: '28px', border: '1px solid var(--border)' }}>
                ...
                <div style={{ border: '1px solid var(--border)', borderTop: 'none', background: 'var(--white)', maxHeight: '200px', overflowY: 'auto' }}>
                  {moveResults.map(w => (
                    <div key={w.id} onClick={() => { setSelectedWine(w); setMoveSearch(w.description); setMoveResults([]) }}
                      ...
                      <div style={{ fontSize: '10px', color: 'var(--muted)' }}>{w.vintage} · {w.region} · IB £{parseFloat(w.purchase_price_per_bottle).toFixed(2)} → DP £{calcDP(w.purchase_price_per_bottle, '75')}</div>
                    </div>
                  ))}
                </div>
              )}
            ...
            {selectedWine && (
              <div style={{ background: 'rgba(107,30,46,0.06)', border: '1px solid rgba(107,30,46,0.2)', padding: '12px', marginBottom: '16px' }}>
                ...
                <div style={{ fontSize: '11px', color: 'var(--muted)', marginTop: '4px' }}>IB £{parseFloat(selectedWine.purchase_price_per_bottle).toFixed(2)} → DP £{calcDP(selectedWine.purchase_price_per_bottle, '75')} · {selectedWine.quantity} in bond</div>
                      ...
                      <div style={{ border: '1px solid var(--border)', borderTop: 'none', background: 'var(--white)', maxHeight: '160px', overflowY: 'auto' }}>
                        {scanSearchResults.map(w => (
                          <div key={w.id} onClick={() => selectScanWine(w)}
                            ...
                            <div style={{ fontSize: '10px', color: 'var(--muted)' }}>{w.vintage} · DP £{calcDP(w.purchase_price_per_bottle, '75')}</div>
                          </div>
                        ))}
                      </div>
                    )}
              ...
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
                  <div>
                    ...
                    <input type="number" step="0.01" value={scanIBPrice} onChange={e => setScanIBPrice(e.target.value)} placeholder="0.00" onFocus={e => e.target.select()}
                      style={{ width: '100%', border: '1px solid var(--border)', background: 'var(--white)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '12px', outline: 'none', boxSizing: 'border-box' }} />
                    {scanIBPrice && <div style={{ fontSize: '10px', color: 'var(--wine)', marginTop: '3px', fontFamily: 'DM Mono, monospace' }}>DP £{calcDP(parseFloat(scanIBPrice), scanBottleSize)}</div>}
                  </div>
                  <div>
                ...
                <div style={{ marginBottom: '12px' }}>
                  ...
                  {scanSalePrice && scanIBPrice && (() => {
                    const dp = calcDP(parseFloat(scanIBPrice), scanBottleSize)
                    const margin = ((parseFloat(scanSalePrice) - parseFloat(dp)) / parseFloat(dp) * 100)
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
              <div>
                ...
                <input type="number" step="0.01" value={addIBPrice} onChange={e => updateAddField('addIBPrice', e.target.value)} onFocus={e => e.target.select()} placeholder="0.00"
                  style={{ width: '100%', border: '1px solid var(--border)', background: 'var(--white)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '12px', outline: 'none', boxSizing: 'border-box' }} />
                {addIBPrice && <div style={{ fontSize: '10px', color: 'var(--wine)', marginTop: '3px', fontFamily: 'DM Mono, monospace' }}>DP £{calcDP(parseFloat(addIBPrice), addBottleSize)}</div>}
              </div>
              <div>
