...
function EditableCell({ value, onSave, type = 'text', step, min, placeholder, style, width }) {
  ...
  return (
    <input type={type} step={step} min={min} value={local} placeholder={placeholder}
...
export default function StudioPage() {
  ...
  async function fetchStudio() {
    setLoading(true)
    const { data, error } = await supabase.from('studio').select('*, wines(*)').order('date_moved', { ascending: false })
  ...
  async function confirmMove() {
    ...
    const { error } = await supabase.from('studio').insert({
      wine_id: selectedWine.id, quantity: moveQty, date_moved: moveDate,
      dp_price: dp, status: 'Available', notes: moveNotes || null, include_in_local: false
    })
  ...
  function openAddModal() {
    setShowAddModal(true); setAddDescription(''); setAddProducer(''); setAddVintage(''); setAddColour('')
    setAddRegion(''); setAddCountry(''); setAddBottleSize('75'); setAddQuantity(1)
    setAddIBPrice(''); setAddSalePrice(''); setAddNotes(''); setAddWineId('')
  }
  ...
  async function saveNewWine() {
    ...
    try {
      ...
      const { error: studioError } = await supabase.from('studio').insert({
        wine_id: newWine.id, quantity: addQuantity, date_moved: new Date().toISOString().split('T')[0],
        dp_price: dp, sale_price: addSalePrice ? parseFloat(addSalePrice) : null,
        status: 'Available', notes: addNotes || null, include_in_local: false,
        bottle_size: addBottleSize, colour: addColour || null,
      })
      if (studioError) throw studioError
      await fetchStudio(); setShowAddModal(false)
  ...
  function openScanModal() {
    ...
    setScanDate(new Date().toISOString().split('T')[0]); setScanNotes(''); setScanVintage('')
    setScanBottleSize('75'); setScanSalePrice(''); setScanColour(''); setScanIBPrice(''); setScanRetailPrice('')
  }
  ...
  async function analyseLabel() {
    ...
    try {
      const base64 = await new Promise((res, rej) => {
        const r = new FileReader(); r.onload = () => res(r.result.split(',')[1]); r.onerror = rej; r.readAsDataURL(scanFile)
      })
      ...
      if (searchTerm) {
        ...
        if (matches && matches.length > 0) {
          ...
          setScanColour(matches[0].colour || '')
        }
      }
    } catch (err) { console.error('Label analysis error:', err); alert('Label reading failed: ' + err.message) }
  ...
  async function saveScanEntry() {
    ...
    const insertData = {
      wine_id: scanWine?.id || null, quantity: scanQty, date_moved: scanDate,
      dp_price: dp, status: 'Available', notes: scanNotes || null, include_in_local: false,
      bottle_size: scanBottleSize || '75', sale_price: scanSalePrice ? parseFloat(scanSalePrice) : null,
      colour: scanColour || null,
    ...
    <div style={{ minHeight: '100vh', background: 'var(--cream)', overflowX: 'hidden' }}>
      ...
      <div style={{ padding: '76px 28px 24px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '16px', flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <button onClick={() => setShowDPTotal(v => !v)}
            style={{ background: showDPTotal ? 'var(--ink)' : 'none', border: '1px solid var(--border)', color: showDPTotal ? '#d4ad45' : 'var(--muted)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '11px', letterSpacing: '0.08em', cursor: 'pointer', whiteSpace: 'nowrap' }}>
        ...
        {(filterColour || sortField !== 'date_moved') && (
          <div style={{ marginBottom: '12px', display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
            <span style={{ fontSize: '10px', color: 'var(--muted)', fontFamily: 'DM Mono, monospace', letterSpacing: '0.08em' }}>
              {filtered.length} wine{filtered.length !== 1 ? 's' : ''}{filterColour ? ` · ${filterColour}` : ''}
            </span>
            {(filterColour || filterStatus || search) && (
              <button onClick={() => { setFilterColour(''); setFilterStatus('Available'); setSearch('') }}
                style={{ background: 'none', border: 'none', fontSize: '10px', color: 'var(--wine)', cursor: 'pointer', fontFamily: 'DM Mono, monospace', padding: 0 }}>Clear filters</button>
            )}
          </div>
        )}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
            <thead>
              <tr style={{ background: 'var(--ink)', color: 'var(--white)' }}>
                {[
                  { label: 'Wine', field: 'name' }, { label: 'Vintage', field: null },
                  { label: 'Colour', field: null }, { label: 'Size', field: null },
                  { label: 'Qty', field: 'quantity' }, { label: 'Moved', field: 'date_moved' },
                  { label: 'DP Price', field: 'dp_price' }, { label: 'Sale £', field: 'sale_price' },
                  { label: 'Status', field: null }, { label: 'On Hand', field: 'local' },
                  { label: 'Notes', field: null }, { label: '', field: null }, { label: '!', field: null },
                ].map(({ label, field }) => (
                  <th key={label} onClick={field ? () => cycleSort(field) : undefined}
                  ...
                  <tr style={{ borderBottom: isDetailOpen ? 'none' : '1px solid #ede6d6', background: s.include_in_local ? 'rgba(45,106,79,0.04)' : 'transparent' }}>
                    ...
                    <td style={{ padding: '9px 12px', whiteSpace: 'nowrap' }}>
                      {isEditing ? (
                        <select defaultValue={s.bottle_size || '75'} onChange={e => updateStudio(s.id, 'bottle_size', e.target.value)}
                          ...
                          <option value="37.5">37.5cl</option><option value="75">75cl</option><option value="150">150cl</option>
                        </select>
                    ...
                    <td style={{ padding: '9px 12px' }}>
                      <EditableCell type="number" min="0" value={s.quantity}
                        ...
                        style={{ border: '1px solid var(--border)', background: 'var(--cream)', padding: '3px 6px', fontFamily: 'DM Mono, monospace', fontSize: '13px', fontWeight: 600, outline: 'none', textAlign: 'center' }} width="52px" />
                    </td>
                    ...
                    <td style={{ padding: '9px 12px' }}>
                      {isEditing ? (
                        <EditableCell type="number" step="0.01"
                          ...
                          onSave={v => updateStudio(s.id, 'dp_price', v)} placeholder="0.00"
                    ...
                    <td style={{ padding: '9px 12px' }}>
                      <EditableCell type="number" step="0.01" value={s.sale_price ? parseFloat(s.sale_price) : null}
                        onSave={v => updateStudio(s.id, 'sale_price', v)} placeholder="—"
                        style={{ border: '2px solid rgba(107,30,46,0.25)', background: 'rgba(107,30,46,0.03)', padding: '3px 6px', fontFamily: 'DM Mono, monospace', fontSize: '12px', outline: 'none', fontWeight: 600, color: s.sale_price ? 'var(--wine)' : 'var(--muted)' }} width="72px" />
                    </td>
                    <td style={{ padding: '9px 12px' }}>
                      {isEditing ? (
                    ...
                    <td style={{ padding: '9px 12px', whiteSpace: 'nowrap' }}>
                      ...
                      {isEditing ? (
                        <button onClick={() => setEditingRow(null)}
                          style={{ background: 'var(--wine)', border: 'none', cursor: 'pointer', color: 'var(--white)', fontSize: '10px', padding: '3px 8px', fontFamily: 'DM Mono, monospace', letterSpacing: '0.05em', marginRight: '4px' }}>Done</button>
                      ) : (
                        <button onClick={() => setEditingRow(s.id)}
                          style={{ background: 'none', border: '1px solid var(--border)', cursor: 'pointer', color: 'var(--muted)', fontSize: '11px', padding: '2px 7px', fontFamily: 'DM Mono, monospace', marginRight: '4px' }}>✎</button>
                      )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(20,15,10,0.7)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: 'var(--cream)', width: '100%', maxWidth: '520px', padding: '28px', border: '1px solid var(--border)' }}>
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              <input value={moveSearch} onChange={e => searchWines(e.target.value)} placeholder="Start typing a wine name…" style={{ width: '100%', border: '1px solid var(--border)', background: 'var(--white)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '12px', outline: 'none', boxSizing: 'border-box' }} />
              {moveResults.length > 0 && !selectedWine && (
                <div style={{ border: '1px solid var(--border)', borderTop: 'none', background: 'var(--white)', maxHeight: '200px', overflowY: 'auto' }}>
                  {moveResults.map(w => (
                    <div key={w.id} onClick={() => { setSelectedWine(w); setMoveSearch(w.description); setMoveResults([]) }} style={{ padding: '10px 12px', cursor: 'pointer', borderBottom: '1px solid #ede6d6' }} onMouseEnter={e => e.currentTarget.style.background = '#f5f0e8'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
              <div>
                ...
                <input type="number" min="1" value={moveQty} onChange={e => setMoveQty(parseInt(e.target.value))} onFocus={e => e.target.select()} style={{ width: '100%', border: '1px solid var(--border)', background: 'var(--white)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '12px', outline: 'none', boxSizing: 'border-box' }} />
              </div>
              <div>
                ...
                <input type="date" value={moveDate} onChange={e => setMoveDate(e.target.value)} style={{ width: '100%', border: '1px solid var(--border)', background: 'var(--white)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '12px', outline: 'none', boxSizing: 'border-box' }} />
              </div>
            </div>
            <div style={{ marginBottom: '20px' }}>
              ...
              <input type="text" value={moveNotes} onChange={e => setMoveNotes(e.target.value)} placeholder="e.g. From case opened for tasting" style={{ width: '100%', border: '1px solid var(--border)', background: 'var(--white)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '12px', outline: 'none', boxSizing: 'border-box' }} />
            </div>
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
              ...
              <button onClick={confirmMove} disabled={!selectedWine || moveSaving} style={{ background: selectedWine ? 'var(--wine)' : '#ccc', color: 'var(--white)', border: 'none', padding: '9px 20px', fontFamily: 'DM Mono, monospace', fontSize: '11px', letterSpacing: '0.1em', textTransform: 'uppercase', cursor: selectedWine ? 'pointer' : 'not-allowed' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(20,15,10,0.7)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: 'var(--cream)', width: '100%', maxWidth: '560px', padding: '28px', border: '1px solid var(--border)', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              {!scanPreview ? (
                <div onClick={() => fileInputRef.current?.click()} style={{ border: '2px dashed var(--border)', padding: '32px', textAlign: 'center', cursor: 'pointer', background: 'var(--white)' }} onMouseEnter={e => e.currentTarget.style.borderColor = 'var(--wine)'} onMouseLeave={e => e.currentTarget.style.borderColor = 'var(--border)'}>
                ...
                <div>
                  ...
                  <button onClick={() => { setScanFile(null); setScanPreview(null); setScanDone(false); setScanRaw(null); setScanMatch(null); setScanWine(null) }} style={{ marginTop: '6px', background: 'none', border: 'none', fontSize: '10px', color: 'var(--muted)', cursor: 'pointer', fontFamily: 'DM Mono, monospace' }}>✕ Retake photo</button>
                </div>
              )}
            </div>
            {scanPreview && !scanDone && (
              <button onClick={analyseLabel} disabled={scanAnalysing} style={{ width: '100%', background: 'var(--ink)', color: '#d4ad45', border: 'none', padding: '12px', fontFamily: 'DM Mono, monospace', fontSize: '11px', letterSpacing: '0.15em', textTransform: 'uppercase', cursor: scanAnalysing ? 'wait' : 'pointer', marginBottom: '16px' }}>
              ...
              <div style={{ marginBottom: '16px' }}>
                ...
                {scanRaw && (
                  <div style={{ background: 'var(--white)', border: '1px solid var(--border)', padding: '10px 12px', marginBottom: '10px' }}>
                    ...
                    <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '15px' }}>{[scanRaw.wine_name, scanRaw.producer].filter(Boolean).join(' — ')}{scanRaw.vintage && <span style={{ color: 'var(--muted)', marginLeft: '8px' }}>{scanRaw.vintage}</span>}</div>
                ...
                {scanWine ? (
                  <div style={{ background: 'rgba(45,106,79,0.06)', border: '1px solid rgba(45,106,79,0.3)', padding: '12px' }}>
                    ...
                    <button onClick={() => { setScanWine(null); setScanMatch(null) }} style={{ marginTop: '8px', background: 'none', border: 'none', fontSize: '10px', color: 'var(--muted)', cursor: 'pointer', fontFamily: 'DM Mono, monospace' }}>✕ Not right — search manually</button>
                  </div>
                ) : (
                  <div style={{ background: 'rgba(192,57,43,0.05)', border: '1px solid rgba(192,57,43,0.2)', padding: '12px' }}>
                    ...
                    <input value={scanSearch} onChange={e => scanSearchWines(e.target.value)} placeholder="Search wine name…" style={{ width: '100%', border: '1px solid var(--border)', background: 'var(--white)', padding: '8px 10px', fontFamily: 'DM Mono, monospace', fontSize: '12px', outline: 'none', boxSizing: 'border-box' }} />
                    {scanSearchResults.length > 0 && (
                      <div style={{ border: '1px solid var(--border)', borderTop: 'none', background: 'var(--white)', maxHeight: '160px', overflowY: 'auto' }}>
                        {scanSearchResults.map(w => (
                          <div key={w.id} onClick={() => selectScanWine(w)} style={{ padding: '8px 12px', cursor: 'pointer', borderBottom: '1px solid #ede6d6' }} onMouseEnter={e => e.currentTarget.style.background = '#f5f0e8'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
              ...
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginBottom: '12px' }}>
                  <div>
                    ...
                    <input type="number" min="1" value={scanQty} onChange={e => setScanQty(parseInt(e.target.value) || 1)} onFocus={e => e.target.select()} style={{ width: '100%', border: '1px solid var(--border)', background: 'var(--white)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '14px', fontWeight: 600, outline: 'none', boxSizing: 'border-box' }} />
                  </div>
                  <div>
                    <label style={{ display: 'block', fontSize: '10px', color: 'var(--muted)', marginBottom: '4px', fontFamily: 'DM Mono, monospace' }}>BOTTLE SIZE</label>
                    <select value={scanBottleSize} onChange={e => setScanBottleSize(e.target.value)} style={{ width: '100%', border: '1px solid var(--border)', background: 'var(--white)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '12px', outline: 'none', boxSizing: 'border-box' }}>
                  ...
                  <div>
                    ...
                    <input type="date" value={scanDate} onChange={e => setScanDate(e.target.value)} style={{ width: '100%', border: '1px solid var(--border)', background: 'var(--white)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '12px', outline: 'none', boxSizing: 'border-box' }} />
                  </div>
                </div>
                {!scanWine && (
                  <div style={{ marginBottom: '12px' }}>
                    <label style={{ display: 'block', fontSize: '10px', color: 'var(--muted)', marginBottom: '4px', fontFamily: 'DM Mono, monospace' }}>VINTAGE</label>
                    <input type="text" value={scanVintage} onChange={e => setScanVintage(e.target.value)} placeholder="e.g. 2021" style={{ width: '100%', border: '1px solid var(--border)', background: 'var(--white)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '12px', outline: 'none', boxSizing: 'border-box' }} />
                  </div>
                )}
                <div style={{ marginBottom: '12px' }}>
                  <label style={{ display: 'block', fontSize: '10px', color: 'var(--muted)', marginBottom: '4px', fontFamily: 'DM Mono, monospace' }}>
                    COLOUR{scanWine && !scanWine.colour ? <span style={{ color: 'var(--wine)', marginLeft: '4px', fontWeight: 400 }}> * not set on this record</span> : ''}
                  </label>
                  <select value={scanColour} onChange={e => setScanColour(e.target.value)} style={{ width: '100%', border: scanColour ? '1px solid var(--border)' : '2px solid rgba(107,30,46,0.3)', background: 'var(--white)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '12px', outline: 'none', boxSizing: 'border-box' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
                  <div>
                    ...
                    <input type="number" step="0.01" value={scanIBPrice} onChange={e => setScanIBPrice(e.target.value)} placeholder="0.00" onFocus={e => e.target.select()} style={{ width: '100%', border: '1px solid var(--border)', background: 'var(--white)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '12px', outline: 'none', boxSizing: 'border-box' }} />
                  ...
                  <div>
                    ...
                    <input type="number" step="0.01" value={scanRetailPrice} onChange={e => setScanRetailPrice(e.target.value)} placeholder="0.00" onFocus={e => e.target.select()} style={{ width: '100%', border: '1px solid var(--border)', background: 'var(--white)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '12px', outline: 'none', boxSizing: 'border-box' }} />
                  </div>
                </div>
                <div style={{ marginBottom: '12px' }}>
                  ...
                  <input type="number" step="0.01" value={scanSalePrice} onChange={e => setScanSalePrice(e.target.value)} placeholder="0.00" onFocus={e => e.target.select()} style={{ width: '100%', border: '2px solid rgba(107,30,46,0.3)', background: 'rgba(107,30,46,0.03)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '14px', fontWeight: 600, outline: 'none', boxSizing: 'border-box', color: 'var(--wine)' }} />
                  {scanSalePrice && scanIBPrice && (() => { const dp = (parseFloat(scanIBPrice) + 3) * 1.2; const margin = ((parseFloat(scanSalePrice) - dp) / dp * 100); return <div style={{ fontSize: '10px', color: margin >= 0 ? '#2d6a4f' : '#c0392b', marginTop: '3px', fontFamily: 'DM Mono, monospace' }}>{margin >= 0 ? '+' : ''}{margin.toFixed(1)}% on DP</div> })()}
                </div>
                <div style={{ marginBottom: '20px' }}>
                  ...
                  <input type="text" value={scanNotes} onChange={e => setScanNotes(e.target.value)} placeholder="optional notes…" style={{ width: '100%', border: '1px solid var(--border)', background: 'var(--white)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '12px', outline: 'none', boxSizing: 'border-box' }} />
                </div>
                <button onClick={saveScanEntry} disabled={scanSaving} style={{ width: '100%', background: 'var(--wine)', color: 'var(--white)', border: 'none', padding: '14px', fontFamily: 'DM Mono, monospace', fontSize: '13px', letterSpacing: '0.15em', textTransform: 'uppercase', cursor: scanSaving ? 'wait' : 'pointer', fontWeight: 600 }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(20,15,10,0.7)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: 'var(--cream)', width: '100%', maxWidth: '580px', padding: '28px', border: '1px solid var(--border)', maxHeight: '92vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: '6px' }}>
              ...
              <button onClick={() => setShowAddModal(false)} style={{ background: 'none', border: 'none', fontSize: '18px', cursor: 'pointer', color: 'var(--muted)', lineHeight: 1 }}>✕</button>
            </div>
            <div style={{ fontSize: '11px', color: 'var(--muted)', fontFamily: 'DM Mono, monospace', marginBottom: '24px' }}>Creates a new wine record + studio entry. Use for holiday finds, shop purchases, Corney &amp; Barrow, etc.</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
              <div style={{ gridColumn: '1 / -1' }}>
                ...
                <input value={addDescription} onChange={e => updateAddField('addDescription', e.target.value)} placeholder="e.g. Chambolle-Musigny 1er Cru Les Amoureuses" style={{ width: '100%', border: '1px solid var(--border)', background: 'var(--white)', padding: '9px 12px', fontFamily: 'Cormorant Garamond, serif', fontSize: '15px', outline: 'none', boxSizing: 'border-box' }} />
              </div>
              <div>
                ...
                <input value={addProducer} onChange={e => updateAddField('addProducer', e.target.value)} placeholder="e.g. Roumier" style={{ width: '100%', border: '1px solid var(--border)', background: 'var(--white)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '12px', outline: 'none', boxSizing: 'border-box' }} />
              </div>
              <div>
                ...
                <input value={addVintage} onChange={e => updateAddField('addVintage', e.target.value)} placeholder="e.g. 2019" style={{ width: '100%', border: '1px solid var(--border)', background: 'var(--white)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '12px', outline: 'none', boxSizing: 'border-box' }} />
              </div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginBottom: '12px' }}>
              <div>
                ...
                <select value={addColour} onChange={e => updateAddField('addColour', e.target.value)} style={{ width: '100%', border: '1px solid var(--border)', background: 'var(--white)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '12px', outline: 'none', boxSizing: 'border-box' }}>
              ...
              <div>
                ...
                <input value={addRegion} onChange={e => updateAddField('addRegion', e.target.value)} placeholder="e.g. Burgundy" style={{ width: '100%', border: '1px solid var(--border)', background: 'var(--white)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '12px', outline: 'none', boxSizing: 'border-box' }} />
              </div>
              <div>
                ...
                <input value={addCountry} onChange={e => updateAddField('addCountry', e.target.value)} placeholder="e.g. France" style={{ width: '100%', border: '1px solid var(--border)', background: 'var(--white)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '12px', outline: 'none', boxSizing: 'border-box' }} />
              </div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
              <div>
                ...
                <select value={addBottleSize} onChange={e => updateAddField('addBottleSize', e.target.value)} style={{ width: '100%', border: '1px solid var(--border)', background: 'var(--white)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '12px', outline: 'none', boxSizing: 'border-box' }}>
                  <option value="37.5">37.5cl (half)</option><option value="75">75cl (bottle)</option><option value="150">150cl (magnum)</option>
                </select>
              </div>
              <div>
                ...
                <input type="number" min="1" value={addQuantity} onChange={e => updateAddField('addQuantity', parseInt(e.target.value) || 1)} onFocus={e => e.target.select()} style={{ width: '100%', border: '1px solid var(--border)', background: 'var(--white)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '12px', outline: 'none', boxSizing: 'border-box' }} />
              </div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
              <div>
                ...
                <input type="number" step="0.01" value={addIBPrice} onChange={e => updateAddField('addIBPrice', e.target.value)} onFocus={e => e.target.select()} placeholder="0.00" style={{ width: '100%', border: '1px solid var(--border)', background: 'var(--white)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '12px', outline: 'none', boxSizing: 'border-box' }} />
                {addIBPrice && <div style={{ fontSize: '10px', color: 'var(--wine)', marginTop: '3px', fontFamily: 'DM Mono, monospace' }}>DP £{((parseFloat(addIBPrice) + 3) * 1.2).toFixed(2)}</div>}
              </div>
              <div>
                ...
                <input type="number" step="0.01" value={addSalePrice} onChange={e => updateAddField('addSalePrice', e.target.value)} onFocus={e => e.target.select()} placeholder="0.00" style={{ width: '100%', border: '2px solid rgba(107,30,46,0.25)', background: 'rgba(107,30,46,0.03)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '12px', outline: 'none', boxSizing: 'border-box', color: 'var(--wine)', fontWeight: 600 }} />
              </div>
            </div>
            <div style={{ marginBottom: '16px' }}>
              ...
              <input value={addNotes} onChange={e => updateAddField('addNotes', e.target.value)} placeholder="e.g. Bought at Justerini & Brooks, Dec 2025" style={{ width: '100%', border: '1px solid var(--border)', background: 'var(--white)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '12px', outline: 'none', boxSizing: 'border-box' }} />
            </div>
            <div style={{ marginBottom: '20px', background: 'rgba(212,173,69,0.08)', border: '1px solid rgba(212,173,69,0.3)', padding: '12px' }}>
              <label style={{ display: 'block', fontSize: '10px', letterSpacing: '0.12em', textTransform: 'uppercase', color: '#8a6f1e', marginBottom: '5px', fontFamily: 'DM Mono, monospace' }}>Wine ID (Xero ref) — auto-generated, edit if needed</label>
              <input value={addWineId} onChange={e => setAddWineId(e.target.value)} placeholder="Fill in fields above to generate…" style={{ width: '100%', border: '1px solid rgba(212,173,69,0.5)', background: 'rgba(255,255,255,0.6)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '13px', fontWeight: 600, letterSpacing: '0.15em', outline: 'none', boxSizing: 'border-box', color: '#8a6f1e' }} />
              <div style={{ fontSize: '10px', color: 'var(--muted)', marginTop: '4px', fontFamily: 'DM Mono, monospace' }}>Format: YY · MM (maker) · WWWW (wine) · C (colour) · S (size)</div>
            </div>
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
              <button onClick={() => setShowAddModal(false)} style={{ background: 'none', border: '1px solid var(--border)', padding: '9px 20px', fontFamily: 'DM Mono, monospace', fontSize: '11px', cursor: 'pointer' }}>Cancel</button>
              <button onClick={saveNewWine} disabled={!addDescription || addSaving} style={{ background: addDescription ? 'var(--ink)' : '#ccc', color: addDescription ? 'var(--white)' : '#999', border: 'none', padding: '9px 20px', fontFamily: 'DM Mono, monospace', fontSize: '11px', letterSpacing: '0.1em', textTransform: 'uppercase', cursor: addDescription ? 'pointer' : 'not-allowed' }}>
