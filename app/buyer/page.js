...
export default function BuyerPage() {
  ...
  async function fetchWines() {
    ...
    const { data, error } = await supabase
      .from('wines')
      .select('id, description, vintage, colour, region, country, bottle_format, bottle_volume, sale_price')
      ...
      <div style={{ padding: '24px 28px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '24px', flexWrap: 'wrap' }}>
          <input
            ...
            style={{ flex: 1, minWidth: '200px', border: '1px solid var(--border)', background: 'var(--white)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '12px', outline: 'none' }}
          />
        ...
        {filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--muted)' }}>
            <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '22px', fontWeight: 300, marginBottom: '8px', color: 'var(--ink)' }}>No wines match your search</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '16px' }}>
            {filtered.map(w => {
              ...
              return (
                <div key={w.id} style={{ background: 'var(--white)', border: `1px solid ${hearted ? 'var(--wine)' : 'var(--border)'}`, padding: '20px', position: 'relative', transition: 'box-shadow 0.2s, transform 0.15s', cursor: 'default' }}
                  ...
                  <button onClick={() => toggleHeart(w.id)} style={{ position: 'absolute', top: '14px', right: '14px', background: 'none', border: 'none', fontSize: '20px', cursor: 'pointer', lineHeight: 1, transition: 'transform 0.15s' }}
                    onMouseEnter={e => e.currentTarget.style.transform = 'scale(1.2)'}
                    onMouseLeave={e => e.currentTarget.style.transform = 'scale(1)'}>
                    {hearted ? '❤️' : '🤍'}
                  </button>

                  <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '28px', fontWeight: 300, color: 'var(--wine)', lineHeight: 1, marginBottom: '6px' }}>
                    {w.vintage}
                  </div>

                  <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '16px', fontWeight: 400, color: 'var(--ink)', lineHeight: 1.35, marginBottom: '8px', paddingRight: '28px' }}>
                    {w.description}
                  </div>
      ...
      {hearts.size > 0 && (
        <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: 'var(--ink)', color: 'var(--white)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 28px', zIndex: 200 }}>
          <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '18px' }}>
            {hearts.size} wine{hearts.size !== 1 ? 's' : ''} hearted
          </div>
          <button onClick={downloadWishlist} style={{ background: 'var(--wine)', color: 'var(--white)', border: 'none', padding: '10px 20px', fontFamily: 'DM Mono, monospace', fontSize: '11px', letterSpacing: '0.1em', textTransform: 'uppercase', cursor: 'pointer' }}>
            Download Wishlist
          </button>
        </div>
      )}
