...
export default function ConsignmentPage() {
  ...
  async function generateInvoice() {
    ...
    await supabase.from('consignment_sales').update({
      invoiced: true, invoice_ref: ref, invoice_date: today,
    }).in('id', ids)
    await supabase.from('consignees').update({
      next_invoice_number: activeC.next_invoice_number + 1,
    }).eq('id', activeC.id)
  ...
  async function saveConsignee() {
    ...
    await supabase.from('consignees').insert({
      ...
      email: newEmail || null,
      accounts_email: newAccountsEmail || null,
      address: newAddress || null,
      phone: newPhone || null,
      invoice_prefix: newPrefix.toUpperCase(),
  ...
  function closeAddItem() {
    ...
    setItemDesc(''); setItemVintage(''); setItemColour(''); setItemSize('75')
    setItemQty(1); setItemPrice(''); setItemDate(new Date().toISOString().split('T')[0]); setItemNotes('')
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--cream)', overflowX: 'hidden' }}>
      ...
      {/* Nav */}
      <div className="no-print" style={{ background: 'var(--ink)', color: 'var(--white)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 28px', height: '52px', position: 'fixed', top: 0, left: 0, width: '100%', zIndex: 100, boxSizing: 'border-box' }}>
        ...
        <div style={{ display: 'flex', gap: '4px', flexWrap: 'nowrap' }}>
          {[['Inventory', '/admin'], ['Studio', '/studio'], ['Boxes', '/boxes'], ['Labels', '/labels'], ['Buyer', '/buyer'], ['Local', '/local'], ['Consignment', '/consignment']].map(([label, path]) => (
            <button key={path} onClick={() => router.push(path)} style={{ background: path === '/consignment' ? 'rgba(107,30,46,0.6)' : 'none', color: path === '/consignment' ? '#d4ad45' : 'rgba(253,250,245,0.5)', border: 'none', fontFamily: 'DM Mono, monospace', fontSize: '10px', letterSpacing: '0.12em', textTransform: 'uppercase', cursor: 'pointer', padding: '6px 12px', borderRadius: '2px' }}>{label}</button>
          ))}
        </div>
      ...
      <div className="no-print" style={{ padding: '76px 28px 40px' }}>
        ...
        {consignees.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr', gap: '24px', alignItems: 'start' }}>
              ...
              {consignees.map(c => {
                ...
                return (
                  <div key={c.id} onClick={() => setActiveConsignee(c.id)}
                    style={{ padding: '14px 16px', marginBottom: '6px', background: activeConsignee === c.id ? 'var(--white)' : 'transparent', border: activeConsignee === c.id ? '1px solid var(--border)' : '1px solid transparent', cursor: 'pointer', borderLeft: activeConsignee === c.id ? '3px solid var(--wine)' : '3px solid transparent' }}>
                    <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '16px', fontWeight: 500 }}>{c.name}</div>
                    <div style={{ fontSize: '10px', color: 'var(--muted)', fontFamily: 'DM Mono, monospace', marginTop: '2px' }}>{c.invoice_prefix} series</div>
                    {owed > 0 && <div style={{ marginTop: '6px', display: 'inline-block', background: 'rgba(192,57,43,0.1)', color: '#c0392b', fontSize: '10px', fontFamily: 'DM Mono, monospace', fontWeight: 600, padding: '2px 8px', borderRadius: '10px' }}>£{owed.toFixed(2)} owed</div>}
                  </div>
                )
              })}
            </div>
            ...
            {activeC && (
              <div>
                {/* Client header card */}
                <div style={{ background: 'var(--white)', border: '1px solid var(--border)', padding: '20px 24px', marginBottom: '16px' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px' }}>
                    <div>
                      <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '22px', fontWeight: 500 }}>{activeC.name}</div>
                      <div style={{ fontSize: '12px', color: 'var(--muted)', marginTop: '3px' }}>{activeC.contact_name}{activeC.email ? ` · ${activeC.email}` : ''}</div>
                      {activeC.address && <div style={{ fontSize: '12px', color: 'var(--muted)' }}>{activeC.address}</div>}
                    </div>
                    <div style={{ display: 'flex', gap: '28px' }}>
                      {[
                        [totalHeld, 'bottles held'],
                        [`£${totalValue.toLocaleString('en-GB', { maximumFractionDigits: 0 })}`, 'value out'],
                        [`£${uninvoicedTotal.toFixed(2)}`, 'to invoice', uninvoicedTotal > 0],
                      ].map(([val, label, alert]) => (
                        <div key={label} style={{ textAlign: 'right' }}>
                          <div style={{ fontSize: '22px', fontWeight: 600, fontFamily: 'DM Mono, monospace', color: alert ? '#c0392b' : 'var(--ink)' }}>{val}</div>
                ...
                <div style={{ marginBottom: '24px' }}>
                  ...
                  <div style={{ background: 'var(--white)', border: '1px solid var(--border)', overflowX: 'auto' }}>
                    {activeItems.length === 0 ? (
                      <div style={{ padding: '28px', textAlign: 'center', color: 'var(--muted)', fontFamily: 'Cormorant Garamond, serif', fontSize: '16px' }}>No wines consigned yet — add one above.</div>
                    ) : (
                      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                        <thead>
                          <tr style={{ background: 'var(--ink)', color: 'var(--white)' }}>
                            {['Wine', 'Vintage', 'Size', 'Delivered', 'Remaining', 'Your cost', 'Value out', 'Status', ''].map(h => (
                            ...
                            <tr key={item.id} style={{ borderBottom: '1px solid #ede6d6', opacity: item.status !== 'Active' ? 0.5 : 1 }}>
                              <td style={{ padding: '10px 12px', maxWidth: '220px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                  ...
                                  <div>
                                    <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '14px', fontWeight: 500, lineHeight: 1.3 }}>{item.description}</div>
                              ...
                              <td style={{ padding: '10px 12px', fontFamily: 'DM Mono, monospace', fontSize: '12px' }}>{formatMoney(item.dp_price)}</td>
                        ...
                        <tbody>
                          {activeSales.map(s => (
                            <tr key={s.id} style={{ borderBottom: '1px solid #ede6d6', background: !s.invoiced ? 'rgba(192,57,43,0.03)' : 'transparent' }}>
                              ...
                              <td style={{ padding: '9px 12px', fontFamily: 'DM Mono, monospace' }}>{formatMoney(s.price_per_bottle)}</td>
                              <td style={{ padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontWeight: 600 }}>{formatMoney(s.total_value)}</td>
                              <td style={{ padding: '9px 12px' }}>
                                {s.invoiced
                              ...
                              <td style={{ padding: '9px 12px' }}>
                                {s.invoiced && !s.invoice_paid && (
                                  <button onClick={() => markPaid(s.id)} style={{ fontSize: '10px', fontFamily: 'DM Mono, monospace', background: 'none', border: '1px solid var(--border)', cursor: 'pointer', padding: '2px 8px', color: 'var(--muted)' }}>Mark paid</button>
                                )}
        ...
        <div className="no-print" style={{ position: 'fixed', inset: 0, background: 'rgba(20,15,10,0.7)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: 'var(--cream)', width: '100%', maxWidth: '520px', padding: '28px', border: '1px solid var(--border)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginBottom: '16px' }}>
              ...
              <div>
                <label style={{ display: 'block', fontSize: '10px', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: '5px', fontFamily: 'DM Mono, monospace' }}>Your cost (£/btl)</label>
            ...
            <div style={{ marginBottom: '20px' }}>
              ...
              <input value={itemNotes} onChange={e => setItemNotes(e.target.value)} placeholder="e.g. Magnum"
                style={{ width: '100%', border: '1px solid var(--border)', background: 'var(--white)', padding: '9px 12px', fontFamily: 'DM Mono, monospace', fontSize: '12px', outline: 'none', boxSizing: 'border-box' }} />
            </div>
        ...
        <div className="no-print" style={{ position: 'fixed', inset: 0, background: 'rgba(20,15,10,0.7)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: 'var(--cream)', width: '100%', maxWidth: '480px', padding: '28px', border: '1px solid var(--border)' }}>
            ...
            <div style={{ marginBottom: '14px' }}>
              <label style={{ display: 'block', fontSize: '10px', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: '5px', fontFamily: 'DM Mono, monospace' }}>Select wine from consignment</label>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px', marginBottom: '36px', borderTop: '1px solid #ede6d6', paddingTop: '24px' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: '12px', lineHeight: 1.8, fontFamily: 'DM Mono, monospace', color: '#444' }}>
                    {INVOICE_FROM.bank_name}<br />
                    Sort Code: {INVOICE_FROM.sort_code}<br />
                    Account: {INVOICE_FROM.account_number}
                  </div>
                </div>
              </div>
