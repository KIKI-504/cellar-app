    ...
    <div style={{ minHeight: '100vh', background: 'var(--cream)', overflowX: 'hidden' }} onClick={() => { setExpandedPrice(null) }}>
      ...
      <div style={{ padding: '76px 28px 24px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '16px', flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <button onClick={() => setShowMergeModal(true)} style={{ background: 'none', border: '1px solid #2d6a4f', color: '#2d6a4f', padding: '9px 16px', fontFamily: 'DM Mono, monospace', fontSize: '11px', letterSpacing: '0.1em', textTransform: 'uppercase', cursor: 'pointer' }}>🔀 Merge Duplicates</button>
        </div>
      ...
      {showMergeModal && (
        <MergeDuplicatesModal
          ...
          onMerged={async () => { await fetchWines() }}
        />
      )}
    </div>
  )
...
function MergeDuplicatesModal({ wines, onClose, onMerged }) {
  ...
  const COMPARE_FIELDS = [
    ...
    'manual_override_note', 'manual_override_date',
    'include_in_buyer_view',
  ]
  ...
  async function checkReferences(loserId, loserSourceId) {
    try {
      ...
      console.error('Reference count error:', err)
      setReferenceCounts({ studio: 0, box_items: 0, error: err.message })
    }
  }
  ...
  function getMergedValues() {
    ...
    for (const f of COMPARE_FIELDS) {
      ...
      if (choice === 'loser') merged[f] = loser[f]
      else merged[f] = survivor[f]
    }
    return merged
  }
  ...
  async function performMerge() {
    ...
    try {
      ...
      const auditPayload = {
        ...
        migrated_studio_ids: migratedStudioIds,
        migrated_box_item_ids: migratedBoxItemIds,
        field_choices: fieldChoices,
        similarity_reason: similarityReason.trim(),
      }
                  ...
                  {survivorResults.length > 0 && (
                    <div style={{ border: '1px solid var(--border)', borderTop: 'none', background: 'var(--white)', maxHeight: '220px', overflowY: 'auto' }}>
                      {survivorResults.map(w => (
                        <div key={w.id} onClick={() => selectSurvivor(w)}
                          style={{ padding: '8px 10px', cursor: 'pointer', borderBottom: '1px solid #ede6d6' }}
              ...
              {!loser ? (
                <>
                  ...
                  {loserResults.length > 0 && (
                    <div style={{ border: '1px solid var(--border)', borderTop: 'none', background: 'var(--white)', maxHeight: '220px', overflowY: 'auto' }}>
                      {loserResults.map(w => (
                        <div key={w.id} onClick={() => selectLoser(w)}
                          style={{ padding: '8px 10px', cursor: 'pointer', borderBottom: '1px solid #ede6d6' }}
                          onMouseEnter={e => e.currentTarget.style.background = '#f5f0e8'}
                          onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                          <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '14px' }}>{w.description}</div>
          ...
          {survivor && loser && referenceCounts && (
            <div style={{ background: referenceCounts.studio + referenceCounts.box_items > 0 ? 'rgba(184,148,42,0.08)' : 'rgba(45,106,79,0.06)', border: `1px solid ${referenceCounts.studio + referenceCounts.box_items > 0 ? 'rgba(184,148,42,0.3)' : 'rgba(45,106,79,0.2)'}`, padding: '12px 14px', marginBottom: '16px', fontSize: '11px', fontFamily: 'DM Mono, monospace' }}>
              <div style={{ letterSpacing: '0.1em', textTransform: 'uppercase', color: referenceCounts.studio + referenceCounts.box_items > 0 ? '#7a5e10' : '#2d6a4f', marginBottom: '6px' }}>
                {referenceCounts.studio + referenceCounts.box_items > 0 ? '⚠ Migration required' : '✓ No references to migrate'}
              </div>
              <div style={{ color: 'var(--ink)' }}>
                Loser has <strong>{referenceCounts.studio}</strong> studio entr{referenceCounts.studio === 1 ? 'y' : 'ies'} and <strong>{referenceCounts.box_items}</strong> box item{referenceCounts.box_items === 1 ? '' : 's'} that will be reassigned to the survivor.
              </div>
            </div>
          )}
