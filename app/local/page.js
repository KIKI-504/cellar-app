...
export default function LocalPage() {
  ...
  // NOTE FOR MOBILE MIGRATION: this grid uses 10 columns for laptop.
  // When migrating to mobile, Region + Country columns should collapse into
  // a sub-line under the wine name (already shown there too as fallback).
  // gridTemplateColumns to swap: '2fr 90px 90px 60px 60px 80px 60px 110px 36px'
  // Mobile version: '1fr 60px 60px 80px 36px' with region/country as text below name.
  const GRID = '2fr 90px 90px 60px 60px 80px 60px 110px 36px'
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--cream)', paddingBottom: wishlistCount > 0 ? '80px' : '40px' }} onClick={() => setTooltip(null)}>
      ...
      <div style={{ background: 'var(--ink)', backgroundImage: 'radial-gradient(ellipse at 30% 50%, rgba(107,30,46,0.4) 0%, transparent 60%)', color: 'var(--white)', padding: '32px 24px 28px' }}>
        ...
        <div style={{ fontSize: '13px', color: 'rgba(253,250,245,0.75)', fontFamily: 'Cormorant Garamond, serif', fontStyle: 'italic' }}>Prices shown include DUTY, VAT and DELIVERY</div>
      </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1px', background: 'var(--border)' }}>
            ...
            {filtered.map(s => {
              ...
              const dotColor = colour?.toLowerCase().includes('red') ? '#8b2535'
                ...
                : colour?.toLowerCase().includes('spark') ? '#a8c4d4'
                : '#aaa'
              const price = getPrice(s)
                ...
                <div key={s.id} style={{ background: 'var(--white)' }}>
                  <div style={{
                    ...
                    <div>
                      ...
                      {buyerNote && (
                        <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '13px', color: 'var(--ink)', lineHeight: 1.55, paddingLeft: '13px', opacity: 0.85, marginTop: '4px' }}>{buyerNote}</div>
                      )}
                      {restaurantSpot && (
                    ...
                    {/* WS UK Avg — price → date → above/below label */}
                    <div style={{ textAlign: 'right', paddingTop: '2px' }}>
                      {wsDp ? (
